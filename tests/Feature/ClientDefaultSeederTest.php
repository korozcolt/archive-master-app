...
it('seeds default client data and leaves only one super admin user', function () {
    ...
    expect(DocumentTemplate::query()->count())->toBe(0);
    expect(PhysicalLocationTemplate::query()->count())->toBe(0);
    expect(PhysicalLocation::query()->count())->toBe(0);
    ...
    expect($user->email)->toBe('ing.korozco@gmail.com')
        ...
        ->and($company->name)->toBe('AGUAS DE SUCRE S.A. E.S.P.');
});
