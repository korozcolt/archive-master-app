...
- **Dos búsquedas reales no encontraban documentos que sí existen**, ambas por comparar contra el título crudo:
  ...
  - **Corrección de la propia normalización:** solo se aplica cuando la consulta tiene guiones, es decir cuando hay un identificador que rearmar. Se midió con `COMUNICACION INTERNA N°0069`, que encuentra el documento `#30943`: quitando el ordinal, «0069» quedaba como número suelto, la tolerancia a erratas lo confundía con 0093, 0089 y 0098, y el documento correcto desaparecía entre 232 resultados —peor que antes del cambio—. Dejando la consulta intacta vuelve a aparecer. Los ordinales se resuelven donde corresponde: al comparar con el título, normalizando ambos lados.
- **El plural no encontraba nada, y los sinónimos no estaban en el repositorio.** Buscar `DECLARACIONES DE RENTA` no devolvía ninguno de los 15 documentos titulados `DECLARACION DE RENTA N°…`; peor aún, la tolerancia a erratas emparejaba «DECLARACIONES» con «ACLARACION» y sacaba documentos sin relación.
  - El índice ya tenía una lista curada de 40 sinónimos —`acta/actas`, `comunicacion/comunicaciones`, `resolucion/resoluciones`…— pero **configurada a mano en el motor, no en el repositorio**. Nadie podía revisarla y una instancia nueva no la heredaría.
  - Las 42 entradas quedan ahora en `config/scout.php`, incluida la que faltaba (`declaracion` ↔ `declaraciones`), y `scout:sync-index-settings` las mantiene alineadas en cada despliegue.
  - La lista es curada a propósito: generar plurales por regla produce basura (`presupuestals`, `estudioss`, `documentoss`), porque muchas de las palabras más frecuentes ya son plurales o son adjetivos.
  - Verificado en producción: `DECLARACIONES DE RENTA` devuelve ahora los `DECLARACION DE RENTA N°…` en los primeros puestos.
- Se descartó, tras medirlo, priorizar las coincidencias de título sobre las de contenido: habría descartado la inmensa mayoría de los resultados útiles. En una muestra, `CARLOS NUÑEZ` tiene 4 coincidencias por título y **396 solo por contenido**; `nit`, 3 frente a **397**. Ese contenido es el texto extraído por OCR de los 45.000 documentos y es justo lo que hace valioso el archivo.
- **El número del documento se deduce aunque no se escriba la coma.** En este archivo los documentos se titulan «TIPO N°NNN», así que cuando alguien escribe un tipo seguido de un número está señalando un documento concreto, no pidiendo texto libre. Antes ese número se diluía: `COMUNICACION INTERNA 0069` devolvía 232 resultados encabezados por egresos, sin el documento correcto por ninguna parte.
  - Ahora las cinco formas de escribirlo devuelven **un único resultado, el correcto**: con el grado pegado (`N°0069`), con tilde en «COMUNICACIÓN», con la ene suelta (`N 0069`), abreviado (`No. 0069`) y con el número pelado (`0069`). Las tildes ya las resolvía Meilisearch por su cuenta.
  - **La marcha atrás es lo que lo hace seguro:** si esa lectura no encuentra nada, se sigue con la búsqueda de siempre. Puede añadir precisión, nunca quitar resultados. Comprobado con `contratos 2024`, que se comporta igual que antes.
  - Efecto colateral bueno: `RESOLUCION 405` pasa de cientos de resultados a **dos**.
