...
class ReportsAnalyticsWidget extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        return Cache::remember($cacheKey, 300, function () {
            ...
            $slaCompliance = $this->calculateComplianceRate($slaData);

            $previousSlaData = $reportService->slaComplianceReport($previousMonthFilters);
            $previousSlaCompliance = $this->calculateComplianceRate($previousSlaData);
    ...
    private function calculateComplianceRate(\Illuminate\Support\Collection $documents): float
    {
        if ($documents->isEmpty()) {
            return 0;
        }

        $compliant = $documents->filter(function ($document) {
            return in_array($document->sla_status?->value, ['running', 'warning', 'closed', 'frozen'], true)
                || ($document->sla_status === null && $document->due_date === null);
        })->count();

        return round(($compliant / $documents->count()) * 100, 1);
    }
